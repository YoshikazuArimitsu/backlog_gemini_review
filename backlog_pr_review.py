...
> **重要 — PR コメントの扱い**
> 「PR コメント（レビューコメント等）」セクションを必ず精査してください。
> コメントの議論によって **「このPRでは対応しない」という合意に達した事項**
> （例: 別チケット・別PRで対応予定、仕様として許容、スコープ外と判断 など）は、
> 指摘事項として挙げないでください。合意なく未解決のままの問題は通常どおり指摘してください。
...
7. **総合評価**: 以下の 5 段階基準に従い **1〜5 の整数のみ** で評価してください。
...
総合評価は **数字 1 文字のみ** を記述してください（説明文・補足は不要です）。
...
- 指摘事項
- 指摘事項（重要なものを 3〜5 件、PR コメントで「このPRでは対応しない」と合意済みの事項は除外すること）
